...
//	std::string sequences[] = { "bag","ball","road" };							//test data for lab4.6
	...
	for (int s = 0; s < NumSeq; s++) {
		...
		std::string inputvideo = dataset_path + "/" + sequences[s] + "/img/"
				+ image_path; //path of videofile. DO NOT CHANGE
		...
		cv::Size frame_size(cap.get(cv::CAP_PROP_FRAME_WIDTH),
				cap.get(cv::CAP_PROP_FRAME_HEIGHT)); //cv::Size frame_size(700,460);
		VideoWriter outputvideo(output_path + "outvid_" + sequences[s] + ".avi",
				CV_FOURCC('X', 'V', 'I', 'D'), 10, frame_size);	//xvid compression (cannot be changed in OpenCV)

		//Read ground truth file and store bounding boxes
		std::string inputGroundtruth = dataset_path + "/" + sequences[s] + "/"
				+ groundtruth_file;	//path of groundtruth file. DO NOT CHANGE
...
//					cv::rectangle(frame, c, Scalar(0, 0, 0));
			...
			//selecting ROI
			cv::Mat croppedOutput = converted_frame(
					list_bbox_est[frame_idx - 1]);
			 ...
			 */
			putText(frame, "Frame " + std::to_string(frame_idx),
					cv::Point(10, 15), FONT_HERSHEY_SIMPLEX, 0.5,
					cv::Scalar(0, 0, 255));
			rectangle(frame, list_bbox_gt[frame_idx - 1], Scalar(0, 255, 0));//draw bounding box for groundtruth
			rectangle(frame, list_bbox_est[frame_idx - 1], Scalar(0, 0, 255));//draw bounding box (estimation)

			Mat manyimages = ShowManyImages("Frames", 4, frame, croppedOutput,
					croppedEstimation, histImage);
...
//			imwrite(frame_filename, frame);
		...
		vector<float> trackPerf = estimateTrackingPerformance(list_bbox_gt,
				list_bbox_est);

		//print stats about processing time and tracking performance
		std::cout << "  Average processing time = "
				<< std::accumulate(procTimes.begin(), procTimes.end(), 0.0)
						/ procTimes.size() << " ms/frame" << std::endl;
		std::cout << "  Average tracking performance = "
				<< std::accumulate(trackPerf.begin(), trackPerf.end(), 0.0)
						/ trackPerf.size() << std::endl;
